...
export default function StudentPassModal({
  ...
  const [className, setClassName] = useState(defaultAttendee?.className || 'K8A1 (2003 — 2006)');
  ...
  const currentAttendee = useMemo(() => {
    if (defaultAttendee && defaultAttendee.fullName.trim().toLowerCase() === name.trim().toLowerCase()) {
  ...
  // Resolve memberId from currentAttendee, defaultAttendee, activeMember, or classRoster
  const effectiveMember = useMemo(() => {
    // 1. Nếu defaultAttendee có memberId
    if (defaultAttendee?.memberId && classRoster.length > 0) {
      const found = classRoster.find((m) => m.id === defaultAttendee.memberId);
      if (found) return found;
    }
    // 2. Nếu currentAttendee có memberId
    if (currentAttendee?.memberId && classRoster.length > 0) {
      const found = classRoster.find((m) => m.id === currentAttendee.memberId);
      if (found) return found;
    }
    // 3. Nếu activeMember đang chọn khớp tên/nickname
    if (activeMember && name) {
      if (
        isVietnameseNameMatch(activeMember, name.trim(), currentNickname) ||
        activeMember.fullName.trim().toLowerCase() === name.trim().toLowerCase()
      ) {
        return activeMember;
      }
    }
    // 4. Tìm kiếm trong classRoster theo họ tên và biệt danh
    if (classRoster.length > 0 && name) {
      const exact = classRoster.find((m) => m.fullName.trim().toLowerCase() === name.trim().toLowerCase());
      if (exact) return exact;
      const alias = classRoster.find((m) => isVietnameseNameMatch(m, name.trim(), currentNickname));
      if (alias) return alias;
    }
    return null;
  }, [name, currentNickname, defaultAttendee, currentAttendee, activeMember, classRoster]);

  const effectiveMemberId = currentAttendee?.memberId || defaultAttendee?.memberId || effectiveMember?.id;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs overflow-y-auto animate-fadeIn">
      ...
      <div className="relative w-full max-w-lg bg-[#FAF8F5] rounded-xl shadow-2xl border border-brand-gold/30 p-5 md:p-7 my-8 text-brand-text">
        ...
        >
          ...
          {/* Header of Card */}
          <div className="flex items-center justify-between border-b border-brand-gold/30 pb-3 mb-4 gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-[9px] font-sans font-bold uppercase tracking-widest text-amber-800 truncate">
                  THPT Thái Nguyên • Khóa 8 (2003 — 2006)
                </p>
                <h4 className="text-sm font-serif font-bold text-brand-text uppercase tracking-wide truncate">
            ...
            <div className="text-right shrink-0">
              ...
              <p className="text-[9.5px] font-mono font-bold text-amber-900 mt-0.5">
                #{passCode}
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className="flex-1 space-y-1.5">
              ...
              {/* Grid: Lớp & Quyền Lợi Áo Polo */}
              <div className="grid grid-cols-2 gap-2 text-xs pt-1.5 border-t border-brand-gold/20">
                <div>
                  <span className="text-[8.5px] uppercase font-sans text-brand-text-muted block font-bold">Lớp / Niên khóa:</span>
                  <span className="font-serif font-bold text-brand-gold text-sm">{className || 'K8A1 (2003 — 2006)'}</span>
                </div>
                <div>
                  <span className="text-[8.5px] uppercase font-sans text-brand-text-muted block font-bold">Quyền lợi đón tiếp:</span>
                  <span className="font-serif font-bold text-brand-text text-sm inline-flex items-center gap-1">
                    <Shirt className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                    <span>Áo Polo Size {normalizeShirtSize(shirtSize)}</span>
                  </span>
                </div>
              </div>
              ...
              <div className="text-[10px] text-brand-text-muted space-y-0.5 pt-1">
                ...
                <p className="flex items-center gap-1">
                  ...
                  <span className="truncate">{eventConfig?.venueName ? `${eventConfig.venueName}${eventConfig.venueAddress ? ` (${eventConfig.venueAddress})` : ''}` : 'Trường THPT Thái Nguyên'}</span>
                </p>
              </div>
            </div>
          </div>
