        ...
        <div className="p-4 sm:p-5 relative z-0 space-y-3.5">
          ...
          <div className="text-center py-2 relative">
            ...
            <div className="mt-2 text-xs font-serif italic text-slate-600">
              {isAttending ? (
                <span className={`font-sans font-semibold text-[11px] inline-flex items-center gap-1 px-2 py-0.5 rounded border ${
                  isConfirmed 
                    ? 'bg-emerald-100 text-emerald-900 border-emerald-300 shadow-2xs font-bold'
                    : 'bg-emerald-50 text-emerald-800 border-emerald-200'
                }`}>
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                  <span>${isConfirmed ? 'Đã xác nhận có mặt chính thức' : 'Xác nhận tham dự hội ngộ trực tiếp'}</span>
                </span>
              ) : (
            ...
            {/* CON DẤU ĐỎ SÁP NIÊM PHONG (WAX SEAL STAMP) KHI ĐÃ XÁC NHẬN THAM GIA */}
            {isConfirmed && (
              <div className="absolute right-0.5 -bottom-2.5 sm:right-2 sm:-bottom-3.5 rotate-[-12deg] pointer-events-none z-20 select-none drop-shadow-xl animate-fadeIn">
                <div className="w-17 h-17 sm:w-21 sm:h-21 rounded-full bg-gradient-to-br from-[#9B1E2D] via-[#7A1523] to-[#4D0A13] border-2 border-amber-300 shadow-2xl flex flex-col items-center justify-center text-center p-1 text-white ring-2 ring-amber-400/40">
                  <div className="w-14.5 h-14.5 sm:w-18.5 sm:h-18.5 rounded-full border border-dashed border-amber-200/80 flex flex-col items-center justify-center p-0.5 bg-gradient-to-tr from-black/25 to-transparent">
                    <Sparkles className="w-3 h-3 text-amber-300 fill-amber-300/40 mb-0.5" />
                    <span className="text-[8.5px] sm:text-[10px] font-sans font-black uppercase tracking-wider leading-none text-amber-200">
                      ĐÃ ĐIỂM DANH
                    </span>
                    <span className="text-[6.5px] sm:text-[7.5px] font-serif font-bold text-amber-300/90 tracking-wider mt-0.5 uppercase">
                      K8A1 • 20 NĂM
                    </span>
                  </div>
                </div>
